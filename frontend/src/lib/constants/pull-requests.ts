// Pull request constants - mirrors backend src/constants/pull-requests.ts
// (Pralumex style: named entries with slug + name).
...
export const MERGE_METHOD_SLUGS = Object.values(MERGE_METHODS).map((m) => m.slug);
