...
// yang disimpan di DB hanya hash SHA-256-nya.
export async function createToken(userId: string, name: string): Promise<{ token: string; id: string }> {
  ...
  const rows = await db.insert(tokens).values({ userId, name, tokenHash: sha256(raw) }).returning();
  const row = rows[0];
...
// Validasi raw token (password dari Basic auth). Update lastUsedAt.
